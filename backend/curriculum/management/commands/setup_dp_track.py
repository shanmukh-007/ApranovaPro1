"""
Django management command to setup DP track
"""
...
class Command(BaseCommand):
    help = 'Setup Data Professional (DP) Track with all projects'

    def handle(self, *args, **options):
        self.stdout.write('Setting up DP Track...')
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f'✅ Created DP Track: {dp_track.name}'))
        else:
            self.stdout.write(self.style.WARNING(f'ℹ️  DP Track already exists: {dp_track.name}'))
        ...
        project1, created = Project.objects.get_or_create(
            ...
            defaults={
                ...
                'order': 1,
                'is_active': True,
                'github_template_repo': 'apranova/dp-project1-template',
                'auto_create_repo': True
            }
        )
        
        if created:
            self.stdout.write(self.style.SUCCESS(f'✅ Created Project 1: {project1.title}'))
        ...
        project2, created = Project.objects.get_or_create(
            ...
            defaults={
                ...
                'order': 2,
                'is_active': True,
                'github_template_repo': 'apranova/dp-project2-template',
                'auto_create_repo': True
            }
        )
        
        if created:
            self.stdout.write(self.style.SUCCESS(f'✅ Created Project 2: {project2.title}'))
        ...
        project3, created = Project.objects.get_or_create(
            ...
            defaults={
                ...
                'order': 3,
                'is_active': True,
                'github_template_repo': 'apranova/dp-project3-template',
                'auto_create_repo': True
            }
        )
        
        if created:
            self.stdout.write(self.style.SUCCESS(f'✅ Created Project 3: {project3.title}'))
        ...
        self.stdout.write('\n' + '='*50)
        self.stdout.write(self.style.SUCCESS('DP CURRICULUM SETUP COMPLETE!'))
        self.stdout.write('='*50)
        self.stdout.write(f'\nTrack: {dp_track.name}')
        self.stdout.write(f'Projects: {Project.objects.filter(track=dp_track).count()}')
        self.stdout.write(f'Total Steps: {ProjectStep.objects.filter(project__track=dp_track).count()}')
        self.stdout.write(f'Total Deliverables: {Deliverable.objects.filter(project__track=dp_track).count()}')
        self.stdout.write('\nProjects:')
        for p in Project.objects.filter(track=dp_track).order_by('number'):
            self.stdout.write(f'  {p.number}. {p.title} ({p.steps.count()} steps, {p.deliverables.count()} deliverables)')
